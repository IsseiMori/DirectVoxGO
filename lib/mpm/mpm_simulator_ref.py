...
class Simulator:
    ...
    def simulate(
        ...
        self.final_grid_m = torch.zeros([model.struct.grid_dim_x, model.struct.grid_dim_y, model.struct.grid_dim_z], dtype=torch.float32, device=self.device, requires_grad=True)
        wp.launch(
            extract_grad_tensor,
            dim=int(
                model.struct.grid_dim_x
            ...
            outputs=[
                wp.torch.from_torch(self.final_grid_m),
            ],
            device=self.device,
        )
...
class App:
    ...
    def update(self):
        ...
        with tape:
            ...
            loss = self.sim.final_grid_m.sum()
    ...
    def render(self):
        if self.renderer:
            ...
            # print(self.state_0.struct)
            # print(self.sim.final_grid_m)
